...
def validar_login():
    password_correcta = obtener_password()
...
class AplicacionCarpinteria:
    ...
    def aplicar_estilos(self):
        ...
        style.map(
            "TNotebook.Tab",
            background=[("selected", ACCENT_WOOD), ("active", "#5c4331")],
            foreground=[("selected", "#1a0f07"), ("active", TEXT_LIGHT)],
        )
        style.configure("TFrame", background=BG_WOOD)
        style.configure(
            "TLabelframe", background=CARD_BG, borderwidth=1, relief="solid"
        )
        style.configure(
            "TLabelframe.Label",
            background=CARD_BG,
            foreground=ACCENT_WOOD,
            font=("Segoe UI", 11, "bold"),
        )
        style.configure(
            "TButton",
            background=ACCENT_WOOD,
            foreground="#1a0f07",
            font=("Segoe UI", 10, "bold"),
            padding=8,
            borderwidth=0,
        ...
        style.configure(
            "Danger.TButton", background=DANGER_RED, foreground=TEXT_LIGHT
        )
        style.map("Danger.TButton", background=[("active", "#d15b49")])
        style.configure(
            "TEntry",
            fieldbackground="#543d2c",
            foreground="#ffffff",
            borderwidth=0,
            padding=5,
        )
        ...
        style.map(
            "Treeview",
            background=[("selected", ACCENT_WOOD)],
            foreground=[("selected", "#1a0f07")],
        )

    # ================= CLIENTES =================
    def crear_interfaz_clientes(self):
        frame_form = ttk.LabelFrame(
            self.tab_clientes, text=" Datos del Cliente ", padding=15
        )
        frame_form.pack(side="left", fill="y", padx=10, pady=10)

        ttk.Label(frame_form, text="Nombre completo:").pack(
            anchor="w", pady=(0, 2)
        )
        self.entry_cli_nombre = ttk.Entry(frame_form, width=25)
        self.entry_cli_nombre.pack(pady=(0, 10))

        ttk.Label(frame_form, text="Teléfono (ej: 0981123456):").pack(
            anchor="w", pady=(0, 2)
        )
        ...
        self.btn_guardar_cli = ttk.Button(
            frame_form,
            text="💾 Guardar / Actualizar",
            command=self.guardar_cliente_gui,
        )
        ...
        ttk.Button(
            ...
            command=self.cargar_cliente_para_editar,
    ...
    def guardar_cliente_gui(self):
        ...
        if self.id_cliente_editando is None:
            ...
            messagebox.showinfo(
                "Éxito", f"Cliente '{nombre}' guardado con éxito."
            )
        else:
            cursor.execute(
                "UPDATE clientes SET nombre=?, telefono=?, direccion=? WHERE"
                " id=?",
                (nombre, telefono, direccion, self.id_cliente_editando),
            )
            messagebox.showinfo(
                "Éxito",
                f"Cliente #{self.id_cliente_editando} actualizado con éxito.",
            )
    ...
    def cargar_cliente_para_editar(self):
        ...
        if not seleccion:
            messagebox.showwarning(
                "Atención", "Selecciona un cliente de la tabla para editar."
            )
            return
    ...
    def eliminar_cliente_gui(self):
        ...
        if not seleccion:
            messagebox.showwarning(
                "Atención", "Selecciona un cliente de la tabla."
            )
            return
        item = self.tabla_clientes.item(seleccion)
    ...
    def crear_interfaz_materiales(self):
        frame_form = ttk.LabelFrame(
            self.tab_materiales, text=" Datos del Insumo ", padding=15
        )
        frame_form.pack(side="left", fill="y", padx=10, pady=10)

        ttk.Label(frame_form, text="Nombre Material:").pack(
            anchor="w", pady=(0, 2)
        )
        ...
        ttk.Label(frame_form, text="Precio unitario ($):").pack(
            anchor="w", pady=(0, 2)
        )
        ...
        ttk.Button(
            frame_form,
            text="💾 Guardar / Actualizar",
            command=self.guardar_material_gui,
        ).pack(fill="x", pady=5)
        ttk.Button(
            frame_form,
            text="🧹 Cancelar / Limpiar",
            command=self.limpiar_form_material,
        ).pack(fill="x", pady=5)

        frame_tabla = ttk.LabelFrame(
            self.tab_materiales, text=" Inventario Actual ", padding=10
        )
        frame_tabla.pack(
            side="right", fill="both", expand=True, padx=10, pady=10
        )

        self.tabla_mat = ttk.Treeview(
            frame_tabla,
            columns=("id", "nombre", "stock", "precio", "alerta"),
            show="headings",
        )
        ...
        ttk.Button(
            frame_tabla,
            text="✏️ Editar Material Seleccionado",
            command=self.cargar_material_para_editar,
        ).pack(anchor="w")
    ...
    def cargar_datos_materiales(self):
        ...
        for m in cursor.fetchall():
            ...
            self.tabla_mat.insert(
                "", "end", values=(m[0], m[1], m[2], f"${m[3]:.2f}", alerta)
            )
        conexion.close()
        ...
        try:
            ...
            if nombre and stock >= 0 and precio >= 0:
                ...
                if self.id_material_editando is None:
                    ...
                    messagebox.showinfo(
                        "Éxito", f"Material '{nombre}' agregado."
                    )
                else:
                    cursor.execute(
                        "UPDATE materiales SET nombre=?, stock=?,"
                        " precio_unitario=? WHERE id=?",
                        (nombre, stock, precio, self.id_material_editando),
                    )
                    messagebox.showinfo(
                        "Éxito",
                        f"Material #{self.id_material_editando} actualizado.",
                    )
                ...
                messagebox.showwarning(
                    "Atención", "Ingrese valores numéricos válidos."
                )
        except ValueError:
            messagebox.showerror(
                "Error", "Stock debe ser entero y Precio decimal."
            )
    ...
    def cargar_material_para_editar(self):
        ...
        if not seleccion:
            messagebox.showwarning(
                "Atención", "Selecciona un material de la tabla para editar."
            )
            return
    ...
    def crear_interfaz_pedidos(self):
        frame_form = ttk.LabelFrame(
            self.tab_pedidos, text=" Datos del Pedido ", padding=15
        )
        frame_form.pack(side="left", fill="y", padx=10, pady=10)
        ...
        ttk.Label(frame_form, text="Descripción del Trabajo:").pack(
            anchor="w", pady=(0, 2)
        )
        self.entry_ped_desc = ttk.Entry(frame_form, width=25)
        self.entry_ped_desc.pack(pady=(0, 10))

        ttk.Label(frame_form, text="Precio Total ($):").pack(
            anchor="w", pady=(0, 2)
        )
        ...
        ttk.Button(
            frame_form,
            text="💾 Guardar / Actualizar",
            command=self.guardar_pedido_gui,
        ).pack(fill="x", pady=5)
        ttk.Button(
            frame_form,
            text="🧹 Cancelar / Limpiar",
            command=self.limpiar_form_pedido,
        ).pack(fill="x", pady=5)

        frame_tabla = ttk.LabelFrame(
            self.tab_pedidos, text=" Gestionar Pedidos Registrados ", padding=10
        )
        frame_tabla.pack(
            side="right", fill="both", expand=True, padx=10, pady=10
        )
        ...
        self.tabla_pedidos = ttk.Treeview(
            frame_tabla,
            columns=("id", "cliente", "desc", "estado", "precio", "fecha"),
            show="headings",
        )
        ...
        ttk.Button(
            frame_acciones,
            text="✏️ Editar",
            command=self.cargar_pedido_para_editar,
        ...
        ttk.Button(
            frame_acciones,
            text="🧾 Recibo (.txt)",
            command=self.generar_recibo_pedido,
        ).pack(side="left", padx=3)
        ttk.Button(
            frame_acciones,
            text="💬 Enviar WhatsApp",
            command=self.enviar_recibo_whatsapp,
        ).pack(side="left", padx=3)
    ...
    def cargar_datos_pedidos(self):
        ...
        for p in cursor.fetchall():
            ...
            self.tabla_pedidos.insert(
                "",
                "end",
                values=(p[0], p[1], p[2], p[3], f"${p[4]:.2f}", fecha_str),
            )
        conexion.close()
        ...
        try:
            ...
            if desc and precio > 0:
                ...
                if self.id_pedido_editando is None:
                    cursor.execute(
                        "INSERT INTO pedidos (cliente_id, descripcion, estado,"
                        " precio_total, fecha) VALUES (?, ?, 'Pendiente', ?,"
                        " ?)",
                        (cliente_id, desc, precio, fecha_hoy),
                    )
                    messagebox.showinfo("Éxito", "Pedido creado con éxito.")
                else:
                    cursor.execute(
                        "UPDATE pedidos SET cliente_id=?, descripcion=?,"
                        " precio_total=? WHERE id=?",
                        (cliente_id, desc, precio, self.id_pedido_editando),
                    )
                    messagebox.showinfo(
                        "Éxito",
                        f"Pedido #{self.id_pedido_editando} actualizado con"
                        " éxito.",
                    )
                ...
                messagebox.showwarning(
                    "Atención", "Complete la descripción y un precio válido."
                )
        except ValueError:
            messagebox.showerror(
                "Error", "ID de cliente debe ser entero y Precio un decimal."
            )
    ...
    def cargar_pedido_para_editar(self):
        ...
        if not seleccion:
            messagebox.showwarning(
                "Atención", "Selecciona un pedido de la tabla para editar."
            )
            return
    ...
    def cambiar_estado_pedido(self):
        ...
        if not seleccion:
            messagebox.showwarning(
                "Atención", "Selecciona un pedido de la tabla."
            )
            return
        ...
        nuevos_estados = {
            "Pendiente": "En Proceso 🛠️",
            "En Proceso 🛠️": "Completado ✅",
...
==================================================
        ...
        messagebox.showinfo(
            "Recibo Creado",
            f"Se ha generado el archivo '{nombre_archivo}' con éxito.",
        )
    ...
    def enviar_recibo_whatsapp(self):
        ...
        if not seleccion:
            messagebox.showwarning(
                "Atención", "Selecciona un pedido de la tabla para enviar."
            )
            return
        ...
        if not resultado or not resultado[0]:
            messagebox.showerror(
                "Error",
                "No se encontró un número de teléfono registrado para este"
                " cliente.",
            )
            return
    ...
    def actualizar_reportes(self):
        ...
        self.lbl_rep_clientes.config(
            text=f"👥 Clientes Registrados: {cli_count}"
        )
...
if __name__ == "__main__":
    ...
    ventana_login.config(bg="#2C1D11")
    ...
    entry_usuario = tk.Entry(
        frame_campos, width=25, font=("Arial", 10), bg="#F5EEE8", relief="flat"
    )
    ...
    # 3. Inicia el ciclo en la ventana de login
